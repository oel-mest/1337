...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/04 21:09:03 by oel-mest          #+#    #+#             */
/*   Updated: 2024/11/04 21:09:03 by oel-mest         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static t_list	*head[10240];
	char			*one_line;

	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	if (read_until_line(&head[fd], fd) == -1)
	{
		ft_lstclear(&head[fd], free);
		return (NULL);
	}
	if (!head[fd])
		return (NULL);
	one_line = malloc(get_endlpos(&head[fd]) + 2);
	if (!one_line)
	{
		ft_lstclear(&head[fd], free);
		return (NULL);
	}
	fetch_oneline(head[fd], one_line);
	update_list(&head[fd]);
	return (one_line);
}
